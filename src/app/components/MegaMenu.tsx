...
// Dinh nghia kieu du lieu day du cho cac item duoc truyen vao
...
export default function MegaMenu({ title, items }: MegaMenuProps) {
  // State de theo doi xem quan nao dang duoc hover
  const [activeDistrict, setActiveDistrict] = useState<DistrictWithWards | null>(null);

  return (
    <div 
      className="relative group"
      // Khi chuot roi khoi toan bo panel, reset de an menu con
      onMouseLeave={() => setActiveDistrict(null)}
    >
      {/* Nut chinh de kich hoat dropdown */}
      ...
      {/* Panel Mega Menu */}
      <div 
        className="origin-top-left absolute left-0 mt-2 w-[40rem] rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200"
      >
        <div className="grid grid-cols-3">
          ...
          <div className="col-span-1 border-r border-gray-200 py-2 max-h-96 overflow-y-auto">
            {items.map((item) => (
              <Link
                key={item.id}
                href={`/van-phong-cho-thue/${slugify(item.name)}`}
                className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 font-semibold"
                // Khi di chuot qua, set quan do la active
          ...
          <div className="col-span-2 py-2 max-h-96 overflow-y-auto">
            {activeDistrict && activeDistrict.wards.length > 0 ? (
              activeDistrict.wards.map((ward) => (
                <Link
                  key={ward.id}
                  href={`/van-phong-cho-thue-phuong/${slugify(ward.name)}-${slugify(activeDistrict.name)}`}
